...
The harness is the middle layer in the SWE Seed system.
...
The harness MUST be self-contained. It MAY be packaged, installed, or regenerated by SWE Seed, but it
MUST NOT require SWE Seed to route work, render skills, manage context, record traces, enforce proof,
or capture learning.

References to SWE Seed in this document define packaging boundaries only. Harness behavior MUST NOT
branch on the presence of SWE Seed.

The harness MUST remain product-domain agnostic. It MAY hand a bounded task to an inner fabrication
layer or receive proof and trace references from one, but it MUST NOT define product seeds, PRDs, SDS
documents, prototype artifacts, or game-specific behavior. Those are fabrication concerns.

Layer integration MUST use stable artifacts and identifiers, not hidden runtime coupling. Shared
interfaces are file paths, route IDs, skill IDs, trace IDs, proof records, context packs, and explicit
handoff packets.

Dependency direction is one-way inward. The harness MAY depend on fabricator handoff artifacts and
proof records when a project enables fabrication, but it MUST NOT depend on SWE Seed. The fabricator
MUST NOT depend on the harness.

## Enterprise Completion Policy

The harness MUST reject unresolved todos, placeholders, mocks, stubs, disabled checks, synthetic
evidence, and unsupported completion claims. These are not acceptable substitutes for implemented
behavior, typed artifacts, eval results, proof records, or reviewed migration notes.

If work is incomplete, the agent MUST record a failed gate, blocked trace, eval issue, regression
case, or ADR. It MUST NOT describe incomplete work as done, enterprise-ready, shippable, validated,
or production-ready.

Completion theater is a blocking defect. A completion claim is allowed only when linked route,
context, implementation, eval, proof, reflection, and adaptation artifacts satisfy the active route
or an explicit human waiver records the skipped checks.
...
- Define BAML as the optional typed artifact compiler for harness artifact generation and
  regeneration.
...
- BAML, installed through `uv` when the typed artifact compiler is enabled,
- native agent hooks,
...
### 3.4 BAML Role in the Harness

BAML is a typed artifact compiler layer for the harness. It defines typed functions and schemas for
generating, validating, and regenerating harness artifacts.

BAML does not replace the harness, route cards, Skill IR, Markdown, or YAML. The relationship is:

- BAML functions produce structured outputs for harness artifacts.
- Renderers convert structured outputs into Markdown or YAML surfaces.
- Markdown and YAML artifacts remain the human-legible operating contracts, agent context,
  validation surfaces, memory artifacts, and regeneration inputs used by agents.
- Generated artifacts MUST remain reviewable, versioned, and testable.

When enabled, BAML MUST be installed through `uv`. The default Python installation path is:
...
The installed BAML runtime and generator version SHOULD be pinned through normal project dependency
management. Generated client code MUST NOT become the source of truth for harness behavior.
...
When packaged in SWE Seed, the harness BAML contract set MUST include:
...
### 3.5 Harness Artifacts Managed by BAML

BAML-managed artifacts are typed intermediate outputs that render into reviewable repository files.

- `RouteCard`: defines how a class of work is routed, what context is required, what skills apply,
  what proof is required, and what done means.
- `SkillIR`: defines a reusable agent behavior or procedure with trigger conditions, inputs,
  outputs, guardrails, and evals.
- `EvalSpec`: defines checks used to determine whether an agent output is acceptable.
- `ContextBudget`: defines what context may be read, what must be read, what is excluded, and how
  raw output is summarized.
- `ContextPack`: defines a bounded, route-specific context bundle prepared for an agent run.
- `HookPolicy`: defines lifecycle events, hook inputs and outputs, blocking behavior, and failure
  handling.
- `PermissionPolicy`: defines actions that are allowed, forbidden, or approval-gated.
- `TraceSchema`: defines the fields required to connect request, route, artifacts, commands, proof,
  and unresolved risks.
- `ReflectionTemplate`: defines the post-run questions and evidence needed to capture useful
  learning.
- `LearningRecord`: defines an approved, rejected, or no-change lesson with provenance.
- `SkillProposal`: defines a proposed new or changed skill with evidence, scope, evals, and rollout
  plan.
- `RegressionCase`: defines a harness failure case that future validation must catch.
- `HarnessADR`: defines a harness architecture decision, alternatives considered, constraints,
  consequences, and rollback path.

### 3.6 Typed Harness Artifact Chain

The typed artifact chain preserves traceability from observed need to harness behavior:
...
-> Reflection
-> LearningRecord
-> SkillProposal or RegressionCase
-> Harness Update
```

Every link SHOULD carry source references to the prior link where practical. Missing links are
allowed only when the route or change is intentionally small, and the skipped link is documented.

### 3.6A Semantic Chain Consistency

When a project enables fabrication, the harness SHOULD preserve the semantic specification chain
owned by the fabrication layer:
...
- routes SHOULD load the smallest upstream semantic artifacts needed for the current proof claim,
- `EvalSpec` remains the source of truth for what must be checked,
- `ProofRecord` remains the source of truth for what actually happened,
- missing upstream or downstream links are validation failures unless an approved waiver exists.

### 3.7 Affordance Control

The harness controls affordances: what actions are easy, difficult, mandatory, or forbidden for the
agent. Implementations SHOULD use route cards, context budgets, hooks, permission policies,
rendered instructions, and validation to:

- make desired actions obvious,
- make unsafe actions blocked or approval-gated,
- make proof mandatory,
- make context bounded,
- make intent visible,
- make scope drift detectable,
- make learning reusable.

These controls MUST be attached to executable behavior or validation. They SHOULD NOT exist as
advice that cannot affect agent action.

### 3.8 Context and Attention Control
...
- what must remain salient during execution.

A `ContextBudget` specifies:
...
Route cards SHOULD reference a `ContextBudget` directly or embed equivalent constraints. If stale or
forbidden context is encountered, the agent MUST treat it as a blocker, warning, or excluded input
according to the selected route.

### 3.9 Intent Preservation

Every route MUST preserve:

- original user or system request,
- desired outcome,
- constraints,
- non-goals,
- definition of done,
...
If intent becomes ambiguous, the agent MUST escalate instead of inventing requirements. Escalation
MAY be a focused user question, a blocked trace entry, or a conservative assumption only when the
route permits assumptions.

### 3.10 Anti-Delusion Gates

The harness MUST reject, block, or flag:

- unsupported completion claims,
- implementation without linked spec or route,
...
Anti-delusion gates MUST emit operator-visible failures or warnings. A warning is acceptable only
when the selected route defines it as non-blocking.

### 3.11 Harness Change Control

Harness changes MUST preserve system coherence.

- Route changes require a reason.
- Skill promotion requires evidence.
- Active skills must be versioned.
- Deprecated skills must remain traceable.
- Regression cases SHOULD be added when a harness failure is discovered.
- Major harness changes SHOULD create a `HarnessADR`.
- Bad harness updates MUST have a rollback path.

No new harness abstraction may be added unless it improves at least one advisor constraint:
prototype speed, proof quality, scope control, context relevance, reuse, user validation, safety, or
learning capture.

### 3.12 Learning Capture

Every substantial agent run MUST produce one of:
...
No substantial run should disappear without memory. A run is substantial when it changes harness
behavior, exposes process friction, requires nontrivial proof, produces reusable learning, or causes
human correction.

### 3.13 Outcome Metrics

Implementations SHOULD track these metrics:

- `Time to Artifact Packet`: elapsed time from task intake to route, context, proof, and artifact
  plan.
- `Time to Prototype`: elapsed time from task intake to first runnable or reviewable implementation.
- `Proof Pass Rate`: percentage of runs whose required proof commands pass without unexplained
  skips.
- `Scope Drift Rate`: percentage of runs that change artifacts outside the routed scope.
- `Context Relevance Score`: operator or automated score for how much loaded context affected the
  next action.
- `Skill Reuse Rate`: percentage of runs that use an existing active skill instead of ad hoc
  procedure.
- `Regression Catch Rate`: percentage of known harness failure cases caught by validation.
- `Human Correction Count`: number of human corrections needed after agent output.
- `Demo Success Rate`: percentage of demos or smoke checks that complete as specified.
- `User Validation Rate`: percentage of runs accepted or confirmed by the user or operator.

Metrics are diagnostic. They MUST NOT override proof gates or human approval.

### 3.14 BAML Function Inventory

BAML functions for the harness layer SHOULD include:

| Function                     | Input artifacts                     | Output artifact        | Purpose                                                       | Validation requirement                                             |
| ---------------------------- | ----------------------------------- | ---------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------ |
| `GenerateRouteCard`          | need, `HarnessADR`, existing routes | `RouteCard`            | Create or revise executable routing behavior.                 | Route schema passes and proof/done fields are present.             |
| `GenerateSkillIR`            | `RouteCard`, traces, reflection     | `SkillIR`              | Create reusable agent procedure.                              | Skill IR schema passes and eval hooks exist.                       |
| `GenerateEvalSpec`           | route, target, constraints          | `EvalSpec`             | Draft local eval checks.                                      | Includes required fields and v0.1 check types.                     |
| `GenerateEvalChecklist`      | `EvalSpec`                          | checklist              | Render human/manual checks.                                   | Maps every required check to evidence.                             |
| `GenerateProofRecord`        | `EvalResult`, observations          | `ProofRecord`          | Record proof evidence.                                        | Maps claims to eval evidence or justified skips.                   |
| `GenerateContextBudget`      | `RouteCard`, specs, memory map      | `ContextBudget`        | Bound required, optional, excluded, and stale context.        | Required files exist and excluded files are enforceable.           |
| `GenerateHookPolicy`         | `RouteCard`, `TraceSchema`, rules   | `HookPolicy`           | Bind lifecycle events to routing, proof, and trace behavior.  | Hook event payloads and blocking rules are explicit.               |
| `GeneratePermissionPolicy`   | safety rules, route needs           | `PermissionPolicy`     | Define allowed, forbidden, and approval-gated actions.        | Unsafe actions have block or approval behavior.                    |
| `GenerateTraceSchema`        | route set, proof model              | `TraceSchema`          | Define trace fields and identifiers.                          | Request, route, proof, risks, and claim fields are required.       |
| `GenerateReflectionTemplate` | `RouteCard`, `TraceSchema`, eval    | `ReflectionTemplate`   | Capture evidence-bearing learning after runs.                 | Template requires eval/proof evidence and no-change option.        |
| `GenerateLearningRecord`     | trace, reflection, proof            | `LearningRecord`       | Distill a run into approved, rejected, or no-change learning. | Record carries provenance and status.                              |
| `GenerateAdaptationDecision` | eval result, reflection, proof      | `AdaptationDecision`   | Decide allowed and blocked learning.                          | Applies the eval-to-adaptation map.                                |
| `ProposeLearningCandidate`   | reflection, eval result, proof      | `LearningCandidate`    | Capture a candidate lesson.                                   | Includes evidence, scope, confidence, and promotion status.        |
| `ProposeSkillFromEvaluation` | learning candidate, evals           | `SkillProposal`        | Propose reusable skill changes from evaluated evidence.       | Proposal includes guardrails, evals, risk, and rollout.            |
| `ProposeRegressionCase`      | failed eval, trace, proof           | `RegressionCase`       | Preserve harness failure as a future test.                    | Case links to eval check and future rule.                          |
| `GenerateHarnessADR`         | need, alternatives, constraints     | `HarnessADR`           | Record major harness decisions.                               | ADR includes decision, consequences, and rollback path.            |
| `RegenerateHarnessArtifacts` | spec, approved artifacts, records   | rendered Markdown/YAML | Rebuild agent-facing artifacts from typed sources.            | Regeneration is deterministic or explains intentional differences. |

Implementations MAY additionally provide:

- `ValidateSemanticChain`: checks semantic-chain links and reports missing links or blocked proof claims.
- `ProposeTraceabilityRepair`: proposes the smallest valid repair for broken semantic-chain links while preserving IDs and source intent.

### 3.15 BAML Non-Goals

- BAML does not replace human approval.
- BAML does not autonomously promote skills.
- BAML does not deploy production changes.
- BAML does not make unverified outputs true.
- BAML does not remove the need for proof gates.
- BAML does not generate abstract artifacts unless they are used by the harness.

### 3.16 Regeneration Contract

The harness spec is one of the three root specs used to regenerate the whole SWE Seed system:
...
An agent regenerating the system from the root specs MUST be able to derive the harness scaffold,
route cards, Skill IR, eval contracts, context policy, proof gates, trace templates, reflection
templates, learning policy, BAML source contracts, and validation expectations from this document
without reading a hidden implementation source.

The harness layer MUST be regenerable from:

- `HARNESS_SPEC.md`,
- `FABRICATOR_SPEC_v0.1.0.md` only where explicit fabricator handoff/proof interfaces are required.

Approved `RouteCard`, `SkillIR`, `EvalSpec`, `HookPolicy`, `LearningRecord`, and `RegressionCase`
files MAY constrain incremental regeneration, but they MUST NOT be required to bootstrap the harness
from the root specs. They are derived or approved operating artifacts, not hidden source material.
...
Regeneration MUST NOT silently overwrite reviewed manual changes. If a renderer would replace a
reviewed artifact, it MUST show the source, diff, and validation result.

### 3.17 Minimal v0.1 Implementation

For v0.1, only implement BAML generation for:
...
The v0.1 implementation is complete when generated artifacts can be reviewed as Markdown/YAML,
validated by deterministic checks, and regenerated without losing explicit human decisions.

### 3.17A Eval Runner and Incomplete-Work Gates

The harness SHOULD expose a local eval-run command surface for deterministic, file-first
evaluation. The canonical command is:

```bash
python scripts/harness.py eval run <spec-path> --output <eval-result-path>
```

Equivalent wrappers MAY exist. If a `justfile` is used, a preferred alias is:

```text
just harness-eval-run <spec> [output]
```

The local eval runner MUST accept object-shaped JSON or YAML EvalSpec files with these required
fields:
...
For v0.1, deterministic local checks SHOULD support at least these check types:
...
Runtime safety lessons learned for local deterministic checks:

- `command_check` MUST run with a bounded timeout and convert timeout expiry into a failing check
  with recorded evidence,
- `git_diff_policy` MUST run with a bounded timeout and report timeout expiry as a validation
  failure instead of hanging the evaluation path,
- check execution failures MUST become check-level failure reasons where practical rather than
  uncaught exceptions.

The eval runner MUST return non-zero status when required checks fail and MUST emit an `EvalResult`
object with check-level statuses and failure reasons where available.

The harness MUST treat unresolved-incomplete markers as release-blocking defects in governed files.
Validation MUST block marker forms configured in the harness validator unless they appear in
explicit policy text describing forbidden behavior.

### 3.18 Advisor Constraint

No new harness abstraction may be added unless it improves at least one of:
...
The reason MUST be stated in the change, proposal, or `HarnessADR`.
...
`docs/specs/` stores source-of-truth specifications. Specification Markdown is a human-legible
operating contract, agent context, validation surface, memory artifact, and regeneration input. It
MUST NOT be treated as informal documentation when it governs agent behavior.
...
- `baml/`
  - BAML schemas and functions for typed harness artifact generation.
  - Source contracts SHOULD be installed and generated with `uv` and `baml-py`.
  - Generated outputs render into reviewable Markdown/YAML artifacts instead of replacing them.
...
## 12A. Evaluation and Adaptation Layer
...
Every substantial run MUST produce or explicitly waive:
...
External tools such as DSPy, LangSmith, pytest, Playwright, custom scripts, manual checklists, or
LLM-as-judge evaluators MAY be used as adapters. They MUST NOT replace the local `EvalSpec` and
`EvalResult` artifacts as the source of truth.

A run MAY propose learning after one successful or failed run. Promotion into an active skill, route,
context policy, eval policy, or harness behavior requires evidence, guardrails, and approval
according to change-control policy.

### 12A.1 Eval Classes

Each substantial run SHOULD evaluate all four classes unless explicitly waived.

- Product outcome evals determine whether the produced artifact works. Examples: artifact exists,
  prototype opens locally, no console errors, game or app behavior works, and required user
  interaction works.
- Process compliance evals determine whether the agent followed route, context, constraints,
  permissions, and proof requirements. Examples: correct route used, context pack respected,
  forbidden files not touched, non-goals preserved, and tests or proof not bypassed.
- Learning quality evals determine whether reflection, lessons, and proposed skills are grounded in
  evidence and not overgeneralized. Examples: reflection cites proof or eval evidence, lesson is
  specific, lesson has scope boundaries, and proposed skill includes trigger, guardrails, and eval.
- Adaptation eligibility evals determine what kind of system change, if any, is justified. Examples:
  promote skill, create regression case, update context budget, update route card, update eval for
  future run, or no change.

### 12A.2 Eval Immutability

The active `EvalSpec` is frozen after implementation handoff. The agent MUST NOT modify the active
eval, pass conditions, proof gates, or constraints during the same run.

If an eval is wrong, incomplete, or too weak, the agent MUST create one of:

- `EvalIssue`
- `RegressionCase`
- `HarnessADR`
- proposed future `EvalSpec` revision

The fix applies to future runs, not the current run, unless a human explicitly waives or restarts the
run. This prevents reward hacking, test weakening, and completion theater.

### 12A.3 Eval Artifacts

`EvalSpec` is the source of truth for what should be checked.

Required `EvalSpec` fields:
...
- `target`
- `required`
- `rule` or check-specific parameters
- `evidence_required`

Supported v0.1 check types:
...
`EvalResult` records what happened when the eval ran.

Required `EvalResult` fields:
...
`AdaptationDecision` records what learning or system change is allowed, blocked, or deferred.

Required `AdaptationDecision` fields:
...
Allowed adaptation targets:

- `SkillIR`
- `RouteCard`
- `ContextBudget`
- `EvalSpec` for future runs
- `RegressionCase`
- BAML function/prompt
- docs
- no-change record

`RegressionCase` captures a reusable failure that must not recur.

Required `RegressionCase` fields:
...
`LearningCandidate` captures a potential reusable lesson before it becomes an active skill.
...
This map MAY be extended. It SHOULD NOT be loosened without evidence and approval.

### 12A.5 Learning Rate and Change-Control Policy

Artifacts closer to runtime execution may change quickly. Artifacts that govern system behavior must
change slowly and require stronger evidence.
...
- No single run may directly modify active governance or constitutional artifacts.
- A run may produce a proposed change.
- Promotion requires evidence.
- Active evals are slower-changing than reflections and skill proposals.
- Root specs are changed only through explicit decision records and migration notes.
...
### 12A.6 Cascading Pressure

Outer-layer change is allowed when multiple evaluated runs produce repeated pressure.

A cascading pressure exists when:

- the same failure mode appears across multiple runs,
- the same manual correction is repeatedly required,
- a route or skill repeatedly causes drift,
- an eval repeatedly catches the same preventable problem,
- a context budget repeatedly omits necessary context,
- users repeatedly fail the same validation step.

When cascading pressure is detected, the system SHOULD create a `HarnessADR` proposing a slower-layer
change.

Suggested thresholds for solo v0.1:

```yaml
cascading_pressure_thresholds:
  active_skill_change:
    affected_runs: 2
  route_card_change:
    affected_runs: 2
...
Thresholds MAY be adjusted later through change-control.

### 12A.7 Promotion Policy

A learning candidate MAY be stored after one run. A reusable skill MAY be proposed after one run. An
active skill requires evidence, guardrails, evals, and approval.
...
promotion_policy:
  ...
  active_skill:
    required:
      - human_approval
      - at_least_one_regression_case
      - defined_trigger
      - defined_inputs_outputs
      - guardrails_defined
      - eval_defined
```

Future optional threshold:

```yaml
future_active_skill_policy:
  repeated_successes: 3
  failure_count: 0
  regression_cases_pass: true
  human_approval: true
```

### 12A.8 Learning Repository Maintenance

The learning repository MUST NOT accumulate unreviewed or duplicated lessons indefinitely.

Learning artifacts MUST have statuses:
...
- prune duplicate lessons,
- merge overlapping skills,
- deprecate low-value skills,
- reject ungrounded learning candidates,
- require regression cases for active skills,
- review stale skills on a cadence.

Suggested cadence:

```yaml
learning_repository_policy:
  review_cadence: weekly
  stale_skill_review_after_days: 30
```

### 12A.9 External Eval Adapters

External tools are adapters, not authorities.

Supported future adapters MAY include:

- pytest adapter,
- Playwright adapter,
- DSPy adapter,
- LangSmith adapter,
- manual checklist adapter,
...
DSPy MAY be used to evaluate and optimize LLM-generated artifacts, prompts, BAML functions, and
context-pack or skill-proposal scoring quality.

LangSmith MAY be used for hosted tracing, datasets, run comparison, and dashboarding when the system
has enough runs to justify it.

Do not implement DSPy or LangSmith in v0.1 unless already present.

### 12A.10 BAML Eval Support

BAML MAY generate eval drafts, but the active eval for a run becomes frozen after handoff.

Harness BAML functions SHOULD include:

| Function                     | Input artifacts             | Output artifact      | Purpose                               | Validation requirement                             |
| ---------------------------- | --------------------------- | -------------------- | ------------------------------------- | -------------------------------------------------- |
| `GenerateEvalSpec`           | route, target, constraints  | `EvalSpec`           | Draft local eval checks.              | Includes all required fields and v0.1 check types. |
| `GenerateEvalChecklist`      | `EvalSpec`                  | checklist            | Render human/manual checks.           | Maps every required check to evidence.             |
| `GenerateProofRecord`        | `EvalResult`, observations  | `ProofRecord`        | Record proof evidence.                | Maps claims to eval evidence or justified skips.   |
| `GenerateReflectionTemplate` | `EvalResult`, `ProofRecord` | `ReflectionTemplate` | Capture evidence-grounded reflection. | Requires eval/proof citations.                     |
| `GenerateAdaptationDecision` | eval results, reflection    | `AdaptationDecision` | Decide allowed and blocked learning.  | Applies the eval-to-adaptation map.                |
| `ProposeRegressionCase`      | failed eval, trace, proof   | `RegressionCase`     | Preserve reusable failure.            | Links to eval check and future rule.               |
| `ProposeLearningCandidate`   | reflection, eval result     | `LearningCandidate`  | Capture candidate lesson.             | Includes evidence, scope, and confidence.          |
| `ProposeSkillFromEvaluation` | learning candidate, evals   | `SkillProposal`      | Propose reusable skill.               | Includes guardrails and proposed eval.             |

### 12A.11 Minimal v0.1 Eval Scope

For v0.1, implement only lightweight local eval infrastructure.
...
- vector memory dependency,
- production deployment,
- multi-agent orchestration,
- automatic root spec rewriting,
- eval mutation during active runs.
...
### 19.10 BAML Artifact Compiler Conformance

Required when the BAML layer is enabled:

- BAML dependencies are installed through `uv`.
- BAML source files define typed outputs for the enabled v0.1 artifact set.
- BAML-generated structured outputs render to Markdown/YAML operating contracts.
- Generated artifacts carry source metadata and remain reviewable in git.
- Regeneration preserves explicit human decisions or emits a blocking diff.
- BAML functions cannot bypass route selection, proof gates, trace records, or human approval.
...
- Markdown/YAML artifact handling that treats governed files as operating contracts, agent context,
  validation surfaces, memory artifacts, and regeneration inputs.

### 20.2 Recommended for Practical Use

- BAML installed through `uv` as a typed artifact compiler for route, skill, eval, context,
  reflection, and proposal artifacts.
