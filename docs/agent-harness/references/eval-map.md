...
| `tests/validate-harness.sh`                    | The behavior is cheap to assert deterministically in shell | file presence or CLI output assertion      |
...
- If the assertion is simple and cheap in shell, mirror it in `tests/validate-harness.sh`.
