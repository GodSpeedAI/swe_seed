...
| `harness_improvement` | The harness itself must change                                     | identify the harness gap and desired behavior                              | `python scripts/harness.py validate`      |
| `implementation`      | A stated requirement must become behavior                          | identify the governing requirement                                         | `python scripts/harness.py validate`      |
...
| `research`            | Information is needed before choosing a change                     | define the decision or question research must support                      | `python scripts/harness.py validate`      |
| `review`              | Changes need risk analysis, not immediate edits                    | identify the changed contract or intended outcome                          | `python scripts/harness.py validate`      |
| `skill_authoring`     | A reusable behavioral skill must be created or revised             | define the JTBD and trigger conditions                                     | `python scripts/harness.py render-skills` |
